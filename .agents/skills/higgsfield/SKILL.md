...
1. **Auth header.** `Authorization: Key {HIGGSFIELD_API_KEY}`. Enter the credential in Nebula **Settings → Higgsfield** and save; it is persisted under `apiKeys.HIGGSFIELD_API_KEY` in project-root `settings.json` and takes effect without a restart. Higgsfield issues credentials as a **key + secret pair**; the API expects `Key {key}:{secret}`. The handler does **not** split or recombine anything — it passes whatever you store verbatim after the literal `Key `. If you have both, paste the combined value `key:secret`; if you only have a single token, paste it alone.
