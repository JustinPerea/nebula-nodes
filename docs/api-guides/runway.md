...
## Nodes available in Nebula (7)

| Node (as shown in app) | Node ID | Type | Key inputs | Notable params | Use it for |
|---|---|---|---|---|---|
| Runway Video | `runway-video` | video-gen | `prompt` (text, optional), `image` (optional) | `model` (Gen-4.5 / Gen-4 Turbo / Gen-3a Turbo / Veo 3.1 / Veo 3.1 Fast / Veo 3), `duration` (2–10s), `ratio` (6 options), `seed` | Animate a photo into a clip, or generate a clip from text. Connect an `image` for image-to-video; leave it empty and write a `prompt` for text-to-video (text-only needs Gen-4.5 or a Veo model, and only 16:9 / 9:16). |
| Runway Aleph | `runway-aleph` | video-gen | `video` (required), `prompt` (required), `reference` (image, optional) | `seed` | Restyle / edit existing footage from a prompt — swap the setting, lighting, or style while keeping the motion. Optional reference image steers the look. (Runs Gen-4 Aleph.) |
| Runway Image | `runway-image` | image-gen | `prompt` (required), `images` (reference images, optional, multiple) | `model` (Gen-4 Image / Gen-4 Image Turbo / Gemini 2.5 Flash), `ratio` (11 options), `seed` | Generate a still image from text, optionally guided by up to 3 reference images. |
...
**Where the nodes appear.** Runway nodes live in the node palette under their media categories — the four video nodes (`runway-video`, `runway-aleph`, `runway-act-two`) under **video-gen**, `runway-image` under **image-gen**, and the three voice nodes (`runway-tts`, `runway-sts`, `runway-dubbing`) under **audio-gen**. Drag a node onto the canvas, wire inputs into its ports, set its params in the inspector, and run.
...
Get the key from the Runway developer dashboard at dev.runwayml.com. All seven nodes share this one key. Every Runway job is asynchronous — Nebula submits the task and polls until it finishes, so a video node may take a minute or two before the output port lights up. Inputs that aren't already public HTTPS URLs are sent inline (local images/clips are encoded automatically), so very large source videos can take longer to upload.
...
| Text → video (`POST /v1/text_to_video`) | Yes | full | `runway-video` with no image. Nebula restricts text-only to Gen-4.5 / Veo models and 16:9 or 9:16, matching API model limits. |
| Video → video / restyle (`POST /v1/video_to_video`) | Yes | partial | `runway-aleph` exposes Gen-4 Aleph (video + prompt + 1 reference image). The API also accepts a Seedance 2 video-to-video path (multi-image + video references, output count) that Nebula does not expose. |
| Text → image (`POST /v1/text_to_image`) | Yes | partial | `runway-image` covers Gen-4 Image, Turbo, and Gemini 2.5 Flash. Reference-image **tags** (`@name` prompt references) and the `contentModeration` knob are not surfaced as node params. |
...
| Image upscale (`POST /v1/image_upscale`) | Yes | none | Enhance / upscale a still image — no Nebula node. |
...
Coverage: ~65% of the Runway **generation** API surface (the endpoints that produce media — 8 of ~12) is exposed in Nebula; counting the full resource surface including account, voices, avatars, workflows, and document management, it's closer to ~40%.

Notable unused capabilities: **sound-effect generation**, **voice isolation/clean-up**, **image upscaling**, **custom voice cloning** (the Voices API — would let TTS/STS use a user's own cloned voice instead of presets), **avatars / avatar videos**, **saved workflows**, and **realtime sessions**. The most natural near-term adds for a media-gen tool are a Sound Effect node, an Image Upscale node, and a Voice Isolation node — all single-input, single-output generation endpoints that fit the existing node pattern. The richer Seedance 2 paths (multi-reference video-to-video) and text-to-image **tagged references** are partial gaps within nodes that already exist.

## Agent skill coverage

**A complete skill exists** at `.claude/skills/runway/SKILL.md` (refreshed 2026-06-04) — a well-structured set: `SKILL.md` (universal rules, an endpoint-picker table mapping every `runway-*` node to its endpoint, response parsing, and an auth-failure playbook) plus four deep-dive references: `video.md`, `image.md`, `character-performance.md`, `audio.md`. It covers all **7** shipped Runway nodes, giving an agent the node IDs and ports, wire-format field names (camelCase), per-node params, wiring/chaining rules, and capability boundaries — so an agent can configure and run all of them without reading the handler.

What it covers:
- **All 7 nodes** (`runway-video`, `runway-aleph`, `runway-image`, `runway-act-two`, `runway-tts`, `runway-sts`, `runway-dubbing`) — per-model ratio/duration matrices, voice-preset lists, and dubbing language codes, all corrected to the real node enums (e.g. the image-ratio count fixed to the node's 11, the bogus `seedance2` option removed).
...
- **Capability boundaries** — a note on what's exposed in Nebula vs. API-only (SFX, upscale, voice cloning, avatars, workflows have no node), so an agent doesn't promise a capability with no node behind it.
