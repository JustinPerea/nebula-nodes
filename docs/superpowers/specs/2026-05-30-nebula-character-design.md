...
- On run, the handler **expands the bundle**: `image_urls = bundle.referenceViews (verbatim) ++ per-use override_refs`, capped at the model's max-ref count; `prompt = "{bundle.frozenTraitString}. {userPrompt}"` (verbatim trait string); `seed = bundle.seed`; apply `consistencyStrength`.
