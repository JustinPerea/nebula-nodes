# Hephaestus — Hermes Agent Integration (Design Spec)
...
**Goal:** Ship Hephaestus as a user-selectable alt-agent in the nebula-nodes chat panel — an iterative, self-critiquing creative generalist powered by Hermes Agent + Kimi K2.6 — alongside the existing Claude chat flow.

---

## 1. Submission Narrative

**Hephaestus = "the forge-god made into a creative agent."** A Hermes-powered chat agent that:
...
- **Axis A (hero):** Iterative self-critique — Hephaestus watches its own output, iterates until quality gates pass.
...
1. Open nebula-nodes. Toggle chat-panel agent selector → **Hephaestus**. Subtitle: *"Powered by Hermes Agent · Kimi K2.6."*
2. User: *"Build me a looping scene: foggy forest at dawn, camera drifts through. Cinematic."*
3. Hephaestus narrates plan. Creates text-input + Imagen + Veo 3.1 (looping) + Preview nodes live on canvas. Applies the existing video-looping heuristic (first-frame = last-frame).
4. First pass runs. Hephaestus calls `vision_analyze` on last-vs-first frames: *"Color grade drifted — end is cooler than the start. That breaks the loop."*
5. Traces defect to stage 1 (source prompt), rewrites Imagen prompt with color language baked in, reruns. Clean loop plays.
6. Hephaestus: *"Saved a learning: `color grade drifted end-to-start on Veo 3.1 looping clips — always apply color via prompt on the source frame, not post.` Next time I'll remember."*
7. Cut to `~/.hermes/skills/hephaestus-learnings/LEARNINGS.md` showing the fresh entry.
...
─────────────────                      ───────────────────────                   ───────────────────────
[Agent selector: Claude|Hephaestus] ─► POST /api/chat {agent, message,           
...
| A3 | Hephaestus skills ship in repo at `nebula-nodes/.hermes/skills/hephaestus-core/` | OSS parity: cloning the repo + running setup gives every user the same baseline Hephaestus. Read-only via Hermes `external_directories`. |
| A4 | Learnings live in writable `~/.hermes/skills/hephaestus-learnings/LEARNINGS.md` | Personal per-user; agent writes via `skill_manage patch`; upstream contribution via user-authored PRs (learnings graduate into shipped skill over time). |
| A5 | Persona rides inside `hephaestus-core` skill (top of SKILL.md) | Non-interactive `-q` mode doesn't honor `/personality` slash commands, but skills preloaded via `--skills hephaestus-core` ARE loaded. User's default SOUL.md stays untouched; Hephaestus's character is scoped to sessions that load the skill. |
| A6 | Autonomy toggle (Auto-pilot vs Step Approval) as header toggle in chat panel | Maps to `HEPHAESTUS_APPROVAL=auto\|step` env var passed to each subprocess turn; SKILL.md instructs pause-on-expensive-operation when step mode is on. |
| A7 | Hephaestus uses `terminal` tool to call `nebula` CLI for MVP | No custom tool registration needed; pattern-parity with how Claude calls nebula today. Upgrade to typed `nebula_*` tools if latency/visibility suffers. |
...
- `approval_request` — emitted when Hephaestus pauses for user approval in Step mode; payload: `{summary, cost_estimate, action_plan}`
- `learning_saved` — emitted when Hephaestus appends to LEARNINGS.md mid-turn; payload: `{topic, entry_preview}`
- Internal: spawn `hermes chat -q MSG -Q --resume SESSION --provider openrouter --model MODEL --skills hephaestus-core`, set `HEPHAESTUS_APPROVAL=auto|step` env var, capture stdout, parse markers (`APPROVAL_REQUIRED:`, `LEARNING_SAVED:`), emit events accordingly.
...
- Add `AGENT_RUNNERS = {"claude": run_claude, "hephaestus": run_hermes}` dispatch at top
...
- Validation: if `agent=hephaestus` and `hermes` binary missing → 503 with `{detail: "Hermes Agent not installed. See docs/HERMES-SETUP.md"}`
- Dispatch: `runner = AGENT_RUNNERS[agent]` then iterate

### Modified: `frontend/src/components/panels/ChatPanel.tsx`

- Header row: agent selector (Claude / Hephaestus), autonomy toggle (Auto / Step)
- Hephaestus visual treatment: copper/forge accent color (e.g. `#b87333`) vs Claude's indigo
...
- `nebula-nodes/.hermes/skills/hephaestus-core/SKILL.md` (persona directive at top, playbook below)
- `nebula-nodes/.hermes/skills/hephaestus-core/metadata.json`
- `docs/HERMES-SETUP.md` — user-facing install + configure walkthrough

### Explicit non-goals (MVP — post-hackathon if time)

- Streaming token-by-token output from Hephaestus (CLI mode returns complete turn; upgrade via gateway adapter later)
- Cross-agent handoff mid-conversation (no "hand this to Hephaestus" mid-thread)
- Cron-scheduled Hephaestus jobs (gateway-only feature)
...
## 4. Data Flow — One Hephaestus Turn

### Auto-pilot mode

1. User types message in chat panel, sends. Frontend POSTs `{agent: 'hephaestus', message, session_id, autonomy: 'auto'}`.
...
3. Backend spawns:
   ...
   hermes chat -q "<message>" -Q \
     ...
     --skills hephaestus-core
   ```
   Env: `HEPHAESTUS_APPROVAL=auto`.
4. Hermes loads: the `hephaestus-core` SKILL (persona + playbook — this is how Hephaestus's character gets in; `--skills` preload works in `-q` non-interactive mode, unlike slash commands), user's default `~/.hermes/SOUL.md` (unchanged — user's global), `AGENTS.md` from nebula CWD, `hephaestus-learnings/LEARNINGS.md` (visible in `skills_list` at session start), relevant `MEMORY.md`.
5. Hephaestus executes turn. Uses:
   ...
   - `skill_manage patch hephaestus-learnings/LEARNINGS.md ...` — appends a learning when a novel insight fires.
...
- `HEPHAESTUS_APPROVAL=step` passed to subprocess
- SKILL.md directive: "when `HEPHAESTUS_APPROVAL=step`, before any execution of a node whose estimated cost >$0.01 OR runtime >30s OR triggers an iteration, pause and print `APPROVAL_REQUIRED: <summary>\nPLAN: <action_plan>\nCOST: <est_cost>\n`. Do not continue the turn."
...
- User clicks Approve → frontend sends a new turn: `"APPROVED: continue."` which Hephaestus resumes from (via `--resume` + continuation prompt)
- User clicks Reject → frontend sends `"REJECTED: <optional edit plan text>"` — Hephaestus adjusts or asks
...
- SKILL.md directive: "at end of each turn that produced insight worth saving, use `skill_manage` to append a timestamped entry to `hephaestus-learnings/LEARNINGS.md`. Entry format:
  ```
...
## 5. Hephaestus Persona & Skill Content

### Persona directive (top of `hephaestus-core/SKILL.md`)

The persona lives AT THE TOP of the `hephaestus-core` skill rather than as a separate personality file. Rationale: `hermes chat -q` non-interactive mode ignores slash commands, so `/personality hephaestus` wouldn't fire — but skills preloaded via `--skills hephaestus-core` ARE loaded, and the agent reads the skill content as authoritative guidance for the session.
...
- At turn start, scan hephaestus-learnings/LEARNINGS.md for entries
...
AUTONOMY MODE (env HEPHAESTUS_APPROVAL):
...
### Playbook sections (body of the same `hephaestus-core/SKILL.md`, below the persona)
...
3. **Vision reliability rules** (geometric-specifics, per-cell for multi-frame, cross-check programmatically where possible — verbatim from hephaestus.html learnings)
...
### `.hermes/skills/hephaestus-learnings/` (writable user-local)

- `LEARNINGS.md` — starts empty (or with a seed entry from hephaestus.html's documented lessons)
- `metadata.json` — `{ "name": "hephaestus-learnings", "description": "Hephaestus's running journal of learnings. Append-only, owned by the agent." }`
...
| Vision call times out | Hermes's own timeout | Hephaestus SKILL.md directs: "if vision_analyze fails, state 'I can't verify this stage' and either proceed with caveat or ask for human review — do not loop infinitely" |
...
1. **First turn, no session** — Hephaestus creates a session, ID persisted
2. **Second turn with session** — resumes correctly; context preserved
3. **Agent switch mid-chat** — flip Claude ↔ Hephaestus; each keeps its own session-id thread
...
6. **Learning save + cite** — force a novel insight → verify LEARNINGS.md gets an entry + `learning_saved` event fires in chat. Run a second related prompt → verify Hephaestus cites the learning
...
- The actual creativity of Hephaestus's output (judged manually during dogfood)
- Rendering quality of models Hephaestus calls (orthogonal)
...
| Day | Work |
|-----|------|
| 1 (today, 2026-04-23) | This spec + plan. End of day: hephaestus-core SKILL.md + hephaestus.md personality drafted. |
| 2 | `run_hermes` + happy-path tests. First end-to-end Hephaestus turn answering "make an image of X" works. |
| 3 | Chat panel agent selector + Hephaestus visual treatment. Autonomy toggle. `approval_request` / `learning_saved` event wiring. |
...
4. **LEARNINGS.md write path** — can `skill_manage` patch inside a skill directory that's user-local (`~/.hermes/skills/hephaestus-learnings/`)? Per research: yes, writable skills go to `~/.hermes/skills/`. Confirm during day 2.
...
- [ ] Chat panel lets user select Hephaestus; Hephaestus responds via Hermes + Kimi K2.6
- [ ] Hephaestus creates nebula nodes via `terminal` tool; canvas updates live
...
- [ ] HERMES-SETUP.md lets a fresh user reach first Hephaestus turn
...
- Gateway PlatformAdapter → streaming chat + cron-scheduled creative tasks + multi-platform Hephaestus (Discord, Telegram) — all unlock from same integration
- Typed `nebula_*` tools via `hermes/tools/registry.py` — better visibility than terminal tool, structured event emission
- Voice mode (ElevenLabs TTS) — Hephaestus narrates its iterative loops out loud
- LEARNINGS → repo-PR flywheel — UI to promote a local learning into an upstream PR on nebula-nodes, growing `hephaestus-core/SKILL.md` over time
- Cross-agent handoff mid-conversation (Claude → Hephaestus on a specific subtask)
