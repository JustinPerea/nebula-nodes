...
- **Iter 5** — `g-document-node`: ⛔ blocked (no present PDF lib; 14-day rule). Then built `g-auto-layout` (PR #11, merged): `computeLayout` (Kahn longest-path) + `autoLayout()` + Toolbar "Layout" button. 364 tests; independent verify PASS; browser confirmed (18-node graph → all 17 edges rightward, 4 columns, undo works). Next: `g-node-groups-color-tags` → `g-cinema-per-shot-backend` → `g-queue-history-manager`.
