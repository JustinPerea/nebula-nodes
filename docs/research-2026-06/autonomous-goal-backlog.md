...
10. `g-queue-history-manager` — ⛔ `blocked` (autonomous) — this is the §2b "History / Provenance / Queue" global surface: a persisted run/job model + cancel/retry/clear + run-history UI. Real product/UX decisions (job granularity: per-run vs per-node; persistence store; cancel/retry semantics) make it unsafe to auto-merge. Parts of its spirit already shipped elsewhere (asset/canvas search, gallery delete, Toolbar cancel-execution). Flag for the user as the natural next human-led surface.
