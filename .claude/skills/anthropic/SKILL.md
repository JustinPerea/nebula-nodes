...
1. **Auth — `x-api-key` header, Settings key `ANTHROPIC_API_KEY`.** The handler reads `api_keys["ANTHROPIC_API_KEY"]`; enter the key in Nebula **Settings → Anthropic** and save. It is persisted under `apiKeys.ANTHROPIC_API_KEY` in project-root `settings.json` and takes effect without a restart. Missing key raises `ValueError("ANTHROPIC_API_KEY is required")`. Note: Anthropic uses `x-api-key`, **not** `Authorization: Bearer`.
