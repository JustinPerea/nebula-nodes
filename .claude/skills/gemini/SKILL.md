...
- **Video extension (Google path, 2026-06-08).** A 4th input port `video` ("Extend Video") continues an existing clip; a 2nd output `source_uri` carries this generation's Veo `files/...` URI. To extend in-graph, wire an upstream `veo-3` node's **`source_uri` → a downstream `veo-3` node's `video`** input. Veo only accepts **its own** generated source URI (re-uploaded/external video → 400 "must be a video generated by VEO"), so wiring the regular `video` (downloaded file) output into `Extend Video` fails by design — use `source_uri`. Extension is `veo-3.0`/`3.1` generate+fast only (not lite, not veo-2.0), output is locked to **720p**, and the source URI is valid only ~2 days. Cannot combine with `image`/`last_frame`.
- **Reference images ("ingredients to video") are NOT available.** Documented for Veo 3.1, but the live Gemini API rejects them ("Your use case is currently not supported") on both `veo-3.1-generate-preview` and `veo-3.1-fast-generate-preview` with an AI-Studio key (verified 2026-06-08). There is no reference-image port; don't promise subject/wardrobe consistency via Veo refs.
