...
- The user configures or builds with any of the 40 FAL-backed Nebula nodes (full list in **Pick the right node**).
...
These describe how Nebula's single shared handler (`backend/handlers/fal_universal.py`) talks to FAL. Every one of the 40 nodes routes through it.
...
3. **Execution pattern (confirmed from the handler):** **queue submit → poll → fetch result** for 38 of 40 nodes. The two exceptions are the GPT Image 2 nodes (`gpt-image-2-fal-generate` and `gpt-image-2-fal-edit`), which use **SSE streaming** (`POST {endpoint}/stream`, `Accept: text/event-stream`) for progressive image preview. Queue flow:
...
All 40 nodes below carry `apiProvider: "fal"` in `backend/data/node_definitions.json`. Endpoint = the slug injected as `endpoint_id`. The right-most column links the matching per-model spec file under `skills/` where one exists (use it for pricing + deep param notes).
...
- **Node ids:** the 40 ids in **Pick the right node** (all carry `apiProvider: "fal"`, `envKeyName: "FAL_KEY"` — except the two `meshy-*` nodes whose `envKeyName` is `["MESHY_API_KEY", "FAL_KEY"]`, preferring direct Meshy).
...
### Capability boundaries (what FAL's API can do that these 40 Nebula nodes CANNOT)

State these plainly when a user asks — do not over-promise.

- **Audio is only partly wired (music — instrumental + vocals).** Music is now wired both ways: `stable-audio-25` (`fal-ai/stable-audio-25/text-to-audio`) reaches text-to-instrumental-music + sound effects (up to 3 min, 44.1kHz, instrumental only — no lyrics/vocals), and `ace-step` (`fal-ai/ace-step`) generates full songs **with synthesized vocals/lyrics** from genre/style `tags` + `lyrics` (up to 60s). The rest of FAL's large audio catalog is **not** reachable: TTS, speech-to-text, voice cloning, dubbing, audio isolation, lipsync (sync-lipsync), video-to-audio/Foley (mmaudio), and stem separation (demucs) — ElevenLabs, MiniMax, Whisper, etc. (Some **video** nodes also bake audio into the clip via `generate_audio`, but that's not standalone audio generation.) For TTS/STS/dubbing in Nebula, use Runway or ElevenLabs nodes instead.
