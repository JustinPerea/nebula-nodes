...
- The user configures or builds with any of the 43 FAL-backed Nebula nodes (full list in **Pick the right node**).
...
These describe how Nebula's single shared handler (`backend/handlers/fal_universal.py`) talks to FAL. Every one of the 43 nodes routes through it.
...
3. **Execution pattern (confirmed from the handler):** **queue submit → poll → fetch result** for 41 of 43 nodes. The two exceptions are the GPT Image 2 nodes (`gpt-image-2-fal-generate` and `gpt-image-2-fal-edit`), which use **SSE streaming** (`POST {endpoint}/stream`, `Accept: text/event-stream`) for progressive image preview. Queue flow:
...
All 43 nodes below carry `apiProvider: "fal"` in `backend/data/node_definitions.json`. Endpoint = the slug injected as `endpoint_id`. The right-most column links the matching per-model spec file under `skills/` where one exists (use it for pricing + deep param notes).
...
- **Node ids:** the 43 ids in **Pick the right node** (all carry `apiProvider: "fal"`, `envKeyName: "FAL_KEY"` — except the two `meshy-*` nodes whose `envKeyName` is `["MESHY_API_KEY", "FAL_KEY"]`, preferring direct Meshy).
...
### Capability boundaries (what FAL's API can do that these 43 Nebula nodes CANNOT)
...
- **Two upscalers / one video-modify model.** `seedvr2-upscale` (faithful) and `clarity-upscaler` (creative/added detail) are the wired upscalers (FAL also has ESRGAN, etc.); `luma-ray2-flash-modify` is the lone video-to-video / restyle node (FAL has many more).
