...
- The user configures or builds with any of the 38 FAL-backed Nebula nodes (full list in **Pick the right node**).
...
These describe how Nebula's single shared handler (`backend/handlers/fal_universal.py`) talks to FAL. Every one of the 38 nodes routes through it.
...
3. **Execution pattern (confirmed from the handler):** **queue submit → poll → fetch result** for 36 of 38 nodes. The two exceptions are the GPT Image 2 nodes (`gpt-image-2-fal-generate` and `gpt-image-2-fal-edit`), which use **SSE streaming** (`POST {endpoint}/stream`, `Accept: text/event-stream`) for progressive image preview. Queue flow:
...
All 38 nodes below carry `apiProvider: "fal"` in `backend/data/node_definitions.json`. Endpoint = the slug injected as `endpoint_id`. The right-most column links the matching per-model spec file under `skills/` where one exists (use it for pricing + deep param notes).
...
- **Node ids:** the 38 ids in **Pick the right node** (all carry `apiProvider: "fal"`, `envKeyName: "FAL_KEY"` — except the two `meshy-*` nodes whose `envKeyName` is `["MESHY_API_KEY", "FAL_KEY"]`, preferring direct Meshy).
...
### Capability boundaries (what FAL's API can do that these 38 Nebula nodes CANNOT)

State these plainly when a user asks — do not over-promise.

- **No audio anywhere.** FAL hosts a large audio catalog (TTS, speech-to-text, music, sound effects, voice cloning, dubbing, audio isolation, lipsync — ElevenLabs, MiniMax, Whisper, Stable Audio, sync-lipsync, etc.). **Zero** of the 38 Nebula FAL nodes target an audio endpoint. The handler can *parse* an audio URL if one came back, but nothing routes there. (Some **video** nodes bake audio into the clip via `generate_audio`, but that's not standalone audio generation.) For TTS/STS/dubbing in Nebula, use Runway or ElevenLabs nodes instead.
...
- **Catalog reach via escape hatch only.** ~45% of FAL's overall capability surface is exposed as dedicated nodes. Anything outside the 38 ids (and outside audio/LLM/training, which need different transport or inputs) is reachable **only** through `fal-universal` by pasting a slug — and only if the model needs nothing beyond a `prompt` and a single `image`.

## Sources

- Nebula audit guide (primary, source-cited): `docs/api-guides/fal.md`.
- Live node roster, ports, params, defaults: `backend/data/node_definitions.json` (38 nodes with `apiProvider: "fal"`).
