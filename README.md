...
features:
  - 123-node BYOK catalog across 14 provider families
  ...
  - 1,200+ tests (894 backend + 322 frontend)
...
<div align="center">
  ...
  <img alt="NODES — 123" src="https://img.shields.io/badge/NODES-123-f3e6c4?style=flat-square&labelColor=0a1612">
...
123 built-in nodes across 14 provider families, plus four **universal nodes** that proxy 300+ more models on OpenRouter, Nous Portal, Replicate, and FAL. Smart caching skips unchanged subgraphs. Streaming nodes show text / video / audio / SVG previews live as they generate.
...
| | |
|---|---|
| **123-NODE CATALOG** | Every major provider as a first-class node. Image, video, 3D, audio, and text generation across 14 provider families — see [CATALOG](#-catalog) for the breakdown. |
...
| **AUDIT DISCIPLINE** | Every API-backed node is verified against canonical provider docs. 1,200+ tests (894 backend + 322 frontend). Live-smoke gate for high-risk handler paths. See [QUALITY](#-quality--audit-discipline). |
...
```
SPEC // 123 NODES · 14 PROVIDER FAMILIES · 11 CATEGORIES
```
...
| **Image generation** | 27 | GPT Image 1 / 1.5 / 2 (direct + FAL, gen + edit), DALL-E 3, Imagen 4, FLUX 1.1 Ultra / Schnell / 2 Pro / Kontext / Fill (Inpaint), Nano Banana (Gemini), Recraft V4 (raster + SVG), Seedream 4.5, Krea 2 / Style Train, Meshy T2I / I2I, Quiver Arrow, Fast SDXL, Runway Image |
...
| **Transform** | 6 | Quiver Arrow Vectorize (raster → SVG), Remove Background, SeedVR2 Upscale, Clarity Upscaler, SeedVR2 Video Upscale, SVG Rasterize |
...
- **1,200+ tests (894 backend + 322 frontend).** Run with `cd backend && python -m pytest` (backend) and `cd frontend && npm run lint && npm run build` (frontend). Body-shape tests pin every direct-provider handler's request envelope against the documented spec.
